...
public final class MolangQueries {
	...
	/**
	 * Update the currently rendering animatable. Should be called via {@link HitboxModel#applyMolangQueries(AnimationState, double) GeoModel.applyMolangQueries} when rendering
	 ...
	 */
	public static void updateActor(AnimationState<? extends AdvancedHitboxEntity> animationState, double animTime) {
		ACTOR = new Actor<>(animationState, animationState.getAnimatable(), animTime, Minecraft.getInstance(), Minecraft.getInstance().level);
	}

	/**
	 * Cleanup method called automatically by {GeoRenderer#defaultRender} to eliminate a memory leak
	 */
	 ...
	 */
	public record Actor<T>(AnimationState<? extends AdvancedHitboxEntity> animationState, T animatable, double animTime, Minecraft mc, Level level) {}
...
//		setDefaultItemQueryValues();
	...
	private static void setDefaultEntityQueryValues() {
		...
		MolangQueries.<Entity>setActorVariable(BODY_Y_ROTATION, actor -> actor.animatable instanceof LivingEntity living ? ((EntityInterface)living).advanced_Hitboxes$commonYBodyRot() : actor.animatable.getViewYRot(actor.animationState.getPartialTick()));
		...
		MolangQueries.<Entity>setActorVariable(RIDER_BODY_Y_ROTATION, actor -> actor.animatable.isVehicle() ? actor.animatable.getFirstPassenger() instanceof LivingEntity living ? ((EntityInterface)living).advanced_Hitboxes$commonYBodyRot() : actor.animatable.getFirstPassenger().getViewYRot(actor.animationState.getPartialTick()) : 0);
